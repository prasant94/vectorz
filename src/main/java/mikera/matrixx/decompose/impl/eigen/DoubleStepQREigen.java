...
public class DoubleStepQREigen {
    ...
    public boolean isZero( int x1 , int x2 ) {
        ...
        return target <= 0.5*EPS*(above+right);
    }
    ...
    public void setup( AMatrix A ) {
        ...
        if( N != A.rowCount() ) {
            ...
            this.A.set(A);
            for(int i=0; i<numStepsFind.length; i++) {
            	numStepsFind[i] = 0;
            }
        }
        ...
        eigenvalues = new Vector2[ A.rowCount() ];
        for( int i = 0; i < eigenvalues.length; i++ ) {
            eigenvalues[i] = new Vector2();
        }
...
//        A.print("%12.3e");
        ...
        if( checkHessenberg && !isHessenberg(A,1,1e-12)) {
            System.out.println(A);
    ...
    public boolean createBulgeSingleStep( int x1 , double eigenvalue ) {
        ...
        double threshold = Math.abs(A.get(x1,x1))*EPS;
    ...
    public boolean bulgeDoubleStepQn( int i ) {
        ...
        double threshold = Math.abs(A.get(i,i))*EPS;
...
//           if( max <= Math.abs(A.get(i,i))*UtilEjml.EPS ) {
        ...
        QRHelperFunctions.rank1UpdateMultR(A,u.asDoubleArray(),gamma,0,i,i+3,_temp.asDoubleArray());
        ...
        QRHelperFunctions.rank1UpdateMultL(A,u.asDoubleArray(),gamma,0,i,i+3);

//        System.out.println("  after Q*A*Q ");
//        A.print();

        if(checkUncountable && A.hasUncountable()) {
            throw new RuntimeException("bad matrix");
        }
...
//                A.print();
        ...
        // apply Q*A  - just do the 3 rows
        QRHelperFunctions.rank1UpdateMultR(A,u.asDoubleArray(),gamma,0,i,i+2,_temp.asDoubleArray());
        ...
        // apply A*Q - just the three things
        QRHelperFunctions.rank1UpdateMultL(A,u.asDoubleArray(),gamma,0,i,i+2);
    ...
    public void eigen2by2_scale( double a11 , double a12 , double a21 , double a22 )
    {
        ...
        if( max == 0 ) {
        	valueSmall.value0.setValues(0, 0);
            valueSmall.value1.setValues(0, 0);
        } else {
            ...
            valueSmall.value0.x *= max;
            valueSmall.value0.y *= max;
            valueSmall.value1.x *= max;
            valueSmall.value1.y *= max;
...
//        System.out.printf("eigen (%6.3f , %6.3f) (%6.3f , %6.3f)\n",p0_real,p0_img,p1_real,p1_img);
    ...
    public void addComputedEigen2x2(int x1,int x2) {
        ...
        if( checkUncountable &&
                (Double.isNaN(valueSmall.value0.x) || Double.isNaN(valueSmall.value1.x)) ) {
            throw new RuntimeException("Uncountable");
        }

        addEigenvalue(valueSmall.value0.x,valueSmall.value0.y);
        addEigenvalue(valueSmall.value1.x,valueSmall.value1.y);
    }

    public boolean isReal2x2( int x1 , int x2 ) {
        eigen2by2_scale(A.get(x1,x1),A.get(x1,x2),A.get(x2,x1),A.get(x2,x2));

        return valueSmall.value0.y == 0; // value0 is real
    }
    ...
    private boolean isHessenberg(Matrix A, int hessenberg, double tol) {
    	if( A.rowCount() != A.columnCount() )
            return false;
